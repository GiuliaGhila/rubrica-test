...
class About extends Component {
  ...
  aggiorna () {    
    axios.post('http://localhost:8080/users/'+ this.props.match.params.id , {nome: this.state.nome, cognome: this.state.cognome, email: this.state.email})
  .then((response) => {
    // handle success
    console.log(response.data.data);
    this.setState({aggiornato: true});
  })
  .catch(function (error) {
    // handle error
    console.log(error);
  ...
  render() {
    ...
    if(this.state.aggiornato) return ( 
      ...
      return(
...

        {
          ...
          <div className="menu-button" onClick={() => {this.aggiorna()}}> aggiorna </div> 
          :null
